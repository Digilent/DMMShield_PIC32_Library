...
uint8_t CALIB_ExportCalibs_Raw(char *pSzCalibs, uint8_t baseAddr);
...
{
    return CALIB_ReadAllCalibsFromEPROM_Raw(&calib, (uint8_t)ADR_EPROM_CALIB);
}
...
uint8_t CALIB_CalibOnZero(double *pMeasuredVal, double *pDispersion, uint8_t fIgnoreDispersion)
{
    ...
    if(bResult == ERRVAL_SUCCESS)
    {
        ...
        if(bResult == ERRVAL_SUCCESS)
        {
            if(!fIgnoreDispersion)
            {
                // check if the measurement dispersion is within accepted range
                bResult = DMM_CheckAcceptedMeasurementDispersion(*pMeasuredVal, DMM_FResistorScale(idxScale)? CALIB_RES_ZERO_REFVAL: 0, pDispersion);
            }
            if(bResult == ERRVAL_SUCCESS)
            {                        
                // check if the calibration data is complete
                CALIB_CheckCompleteCalib(idxScale);  
            }
            else
            {
...
{
    return CALIB_ExportCalibs_Raw(pSzCalibs, (uint8_t)ADR_EPROM_CALIB);
}
...
{
    return CALIB_ExportCalibs_Raw(pSzCalibs, (uint8_t)ADR_EPROM_FACTCALIB);
}
...
**
**	Description:
**		This function is a system function that exports calibration data from a specific location in EPROM.  
**      The export is performed by formatting the calibration values into a text, copied in pSzCalibs, one row for each Scale index. 
**      Each row contains (comma separated) the Scale ID, Multiplicative calibration coefficient and Additive calibration coefficient. 
**      Therefore it is important that the caller of this function allocates enough space in pSzCalibs (750 characters).
...
uint8_t CALIB_ExportCalibs_Raw(char *pSzCalibs, uint8_t baseAddr)
{
    ...
    {
        sprintf(szLine, "%02d, %03.6f, %03.6f\r\n", i, calib1.Dmm[i].Mult, calib1.Dmm[i].Add);
        strcat(pSzCalibs, szLine);
    }
